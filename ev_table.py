...
ALL_ROLLS: list[tuple[int, ...]] = list(combinations_with_replacement(range(1, 7), 5))
N_ROLLS = len(ALL_ROLLS)  # 252
assert N_ROLLS == 252

ROLL_TO_IDX: dict[tuple[int, ...], int] = {r: i for i, r in enumerate(ALL_ROLLS)}
ROLLS_NP = np.array(ALL_ROLLS, dtype=np.int8)  # (252, 5)

# (True = keep, False = reroll) mask index 31 (0b11111) = keep all dice = don't reroll anything
KEEP_MASKS = np.array([[bool((m >> i) & 1) for i in range(5)] for m in range(32)], dtype=bool)
...
_rolls_jnp = jnp.array(ROLLS_NP, dtype=jnp.int32)

SCORE_TABLE = np.array(_score_vmapped(_cats_jnp, _rolls_jnp), dtype=np.int32)  # (252, 13)
...
def get_ev_table() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    # ── rolls_left = 0 ──────────────────────────────────────────────
    ev[:, 0, :] = SCORE_TABLE.astype(np.float32)
